...
public final class FlatteningTransformer implements Transformer {
    ...
    public String description() {
        return "flatten control flow into a switch dispatcher";
    }
    ...
    public void transform(ObfContext ctx) {
        ...
        for (ClassNode cn : ctx.targets(id())) {
            for (MethodNode mn : cn.methods) {
                ...
                try {
                    if (flatten(ctx, cn, mn, rnd)) {
                        flattened++;
                    }
                } catch (RuntimeException | AnalyzerException e) {
                    // Skip methods that cannot be safely flattened.
                }
            }
        }
    ...
    private boolean flatten(ObfContext ctx, ClassNode cn, MethodNode mn, Random rnd)
        ...
        Frame<BasicValue>[] frames = new Analyzer<BasicValue>(new CtxVerifier(ctx, cn))
                .analyze(owner, mn);
        ...
        for (int b = 0; b < blocks.size(); b++) {
            int s = blocks.get(b)[0];
            int e = blocks.get(b)[1];
            for (int i = s; i < e; i++) {
                if (arr[i] instanceof LabelNode ln) {
                    labelBlock.put(ln, b);
                } else {
                    break;
                }
        ...
        for (int s2 = paramSlots; s2 < origMaxLocals; ) {
            switch (cat[s2]) {
                case 1 -> {
                    ...
                    out.add(new VarInsnNode(Opcodes.ISTORE, s2));
                    s2 += 1;
                }
                case 2 -> {
                    out.add(new InsnNode(Opcodes.FCONST_0));
                    out.add(new VarInsnNode(Opcodes.FSTORE, s2));
                    s2 += 1;
                }
                case 3 -> {
                    out.add(new InsnNode(Opcodes.LCONST_0));
                    out.add(new VarInsnNode(Opcodes.LSTORE, s2));
                    s2 += 2;
                }
                case 4 -> {
                    out.add(new InsnNode(Opcodes.DCONST_0));
                    out.add(new VarInsnNode(Opcodes.DSTORE, s2));
                    s2 += 2;
                }
                case 5 -> {
                    out.add(new InsnNode(Opcodes.ACONST_NULL));
                    out.add(new VarInsnNode(Opcodes.ASTORE, s2));
                    s2 += 1;
                }
                default -> s2 += 1;
            }
        ...
        out.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "java/lang/Integer",
        ...
        int ki = 0;
        for (Map.Entry<Integer, LabelNode> en : sw.entrySet()) {
            keys[ki] = en.getKey();
            labs[ki] = en.getValue();
            ki++;
        }
        ...
        for (int b : order) {
            ...
            if (isExit) {
                // terminal return or athrow stays in place
        ...
        probe.tryCatchBlocks = new ArrayList<>();
    ...
    static final class CtxVerifier extends SimpleVerifier {
        ...
        protected boolean isInterface(org.objectweb.asm.Type t) {
            ClassNode c = node(t);
            if (c != null) {
                return (c.access & Opcodes.ACC_INTERFACE) != 0;
            }
            try {
        ...
        protected org.objectweb.asm.Type getSuperClass(org.objectweb.asm.Type t) {
            ClassNode c = node(t);
            if (c != null) {
                return c.superName == null ? null : org.objectweb.asm.Type.getObjectType(c.superName);
            }
            try {
        ...
        protected boolean isAssignableFrom(org.objectweb.asm.Type t, org.objectweb.asm.Type u) {
            ...
            if (tObj && uObj) {
                if (reachable(t.getInternalName(), u.getInternalName())) {
                    return true;
                }
    ...
    private static int catOf(BasicValue v) {
        ...
        if (v == BasicValue.INT_VALUE) {
            return 1;
        }
        if (v == BasicValue.FLOAT_VALUE) {
            return 2;
        }
        if (v == BasicValue.LONG_VALUE) {
            return 3;
        }
        if (v == BasicValue.DOUBLE_VALUE) {
            return 4;
        }
        if (v.isReference()) {
            return 5;
