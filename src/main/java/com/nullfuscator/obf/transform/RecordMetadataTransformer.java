...
public final class RecordMetadataTransformer implements Transformer {
    ...
    public String description() {
        return "atomically hide Java record component schemas";
    }
    ...
    public void transform(ObfContext ctx) {
        ...
        for (ClassNode cn : ctx.targets(id())) {
            ...
            if (cn.fields.stream().anyMatch(GsonSchemaTransformer::hasName)) {
                continue;
            }
            ...
            for (var mn : cn.methods) {
                for (AbstractInsnNode in : mn.instructions.toArray()) {
                    ...
                    for (int i = 0; i < indy.bsmArgs.length; i++) {
                        if (!(indy.bsmArgs[i] instanceof String s)) {
                            continue;
                        }
                        ...
                        if (changed) {
                            indy.bsmArgs[i] = String.join(";", parts);
                        }
                    }
                }
            }
        ...
        ctx.log().debug("recordMetadata: renamed " + components
                + " components across " + records + " records");
    }
