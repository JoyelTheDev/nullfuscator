...
public final class MethodExtractionTransformer implements Transformer {
    ...
    public String description() {
        return "extract static domain methods without leaving proxy bridges";
    }
    ...
    public void transform(ObfContext ctx) {
        ...
        for (ClassNode cn : ctx.classMap().values()) {
            if (cn.superName != null) {
                subclassed.add(cn.superName);
            }
        }
        ...
        for (ClassNode owner : targets) {
            ...
            for (MethodNode mn : new ArrayList<>(owner.methods)) {
                ...
                if ((mn.access & Opcodes.ACC_STATIC) == 0) {
                    owner.access |= Opcodes.ACC_FINAL;
                }
                ...
                String newDesc = (mn.access & Opcodes.ACC_STATIC) != 0
                        ? oldDesc
                        : prependReceiver(owner.name, oldDesc);
                String fresh = "x" + ctx.names().nextRandom(rnd, 8 + rnd.nextInt(9));

                moved.put(key(owner.name, oldName, oldDesc),
                        new Destination(carrier.name, fresh, newDesc));
                    ...
                    if (in instanceof MethodInsnNode call) {
                        ...
                        for (int i = 0; i < indy.bsmArgs.length; i++) {
                            ...
                            if (a instanceof Handle h) {
                                indy.bsmArgs[i] = remapHandle(h, moved, ctx.classMap());
                            } else if (a instanceof ConstantDynamic cd) {
                                indy.bsmArgs[i] = remapCondy(cd, moved, ctx.classMap());
                            }
                        }
                    }
                }
    ...
    private static boolean eligible(ClassNode owner, MethodNode mn, int min, Set<String> subclassed) {
        ...
        if (mn.name.charAt(0) == '<' || mn.name.equals("main")) {
            return false;
        }
        if (mn.name.equals("equals") || mn.name.equals("hashCode") || mn.name.equals("toString")) {
            return false;
        }
        if ((mn.access & Opcodes.ACC_STATIC) == 0
                && (!"java/lang/Object".equals(owner.superName)
                || (owner.interfaces != null && !owner.interfaces.isEmpty())
                || subclassed.contains(owner.name))) {
            return false;
        }
        if (mn.instructions == null || mn.instructions.size() < min) {
            return false;
        }
        if ((mn.visibleAnnotations != null && !mn.visibleAnnotations.isEmpty())
                || (mn.invisibleAnnotations != null && !mn.invisibleAnnotations.isEmpty())) {
            return false;
        }
    ...
    private static ConstantDynamic remapCondy(ConstantDynamic cd, Map<String, Destination> moved,
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            args[i] = a instanceof Handle h ? remapHandle(h, moved, classes)
                    : a instanceof ConstantDynamic nested ? remapCondy(nested, moved, classes)
                    : a;
        }
