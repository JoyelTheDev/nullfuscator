...
public final class GsonSchemaTransformer implements Transformer {
    ...
    public String description() {
        return "preserve selected Gson JSON field names";
    }
    ...
    public static boolean isGsonAnnotation(String desc) {
        return NAME.equals(desc)
                || "Lcom/google/gson/annotations/Expose;".equals(desc)
    ...
    public void transform(ObfContext ctx) {
        ...
        if (include.isEmpty()) {
            throw new IllegalArgumentException("gsonSchema requires explicit include selectors");
        }

        int count = 0;
        for (var cn : ctx.targets(id())) {
            if (!include.matches(cn.name)) {
                continue;
            }
            if ((cn.access & (Opcodes.ACC_ENUM | Opcodes.ACC_RECORD | Opcodes.ACC_INTERFACE)) != 0) {
                continue;
            }
