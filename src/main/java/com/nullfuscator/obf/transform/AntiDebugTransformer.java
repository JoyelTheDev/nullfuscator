...
public final class AntiDebugTransformer implements Transformer {
    ...
    public String description() {
        return "inject debugger/agent detection guards";
    }
    ...
    public void transform(ObfContext ctx) {
        ...
        byte[] src = load();
        if (src == null) {
            ctx.log().warn("antiDebug: DebugCheck resource missing, skipped");
            return;
        }
        ...
        for (int i = 0; i < pool; i++) {
            ...
            if (section.getBoolean("rejectAgents", false)) {
                for (MethodNode method : renamed.methods) {
                    if (method.name.equals("rejectAgents")) {
        ...
        Set<String> usedDetectors = new HashSet<>();
        int guarded = 0;
        ...
        detectors.removeIf(detector -> {
            if (usedDetectors.contains(detector)) {
                return false;
            }
