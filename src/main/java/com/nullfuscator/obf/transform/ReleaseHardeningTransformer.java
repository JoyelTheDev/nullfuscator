...
public final class ReleaseHardeningTransformer implements Transformer {
    ...
    @Override public String id() { return "releaseHardening"; }
    @Override public String description() { return "strip Maven metadata and Fabric descriptions"; }
    ...
    public void transform(ObfContext ctx) {
        ...
        if (section.getBoolean("stripMavenMetadata", true)) {
            ...
            for (String path : ctx.resources().keySet()) {
                if (path.startsWith("META-INF/maven/") || path.startsWith("META-INF/proguard/")) paths.add(path);
            }
        ...
        if (section.getBoolean("stripFabricDescription", true)) {
            for (Map.Entry<String, byte[]> entry : ctx.resources().entrySet()) {
                ...
                if (!path.equals("fabric.mod.json") && !path.endsWith("/fabric.mod.json")) continue;
        ...
        if (removed > 0 || descriptionStripped) ctx.log().debug("releaseHardening removed " + removed
                + " dependency metadata entries" + (descriptionStripped ? " and Fabric descriptions" : ""));
    }
}
