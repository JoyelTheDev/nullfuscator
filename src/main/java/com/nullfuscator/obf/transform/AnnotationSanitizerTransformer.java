...
public final class AnnotationSanitizerTransformer implements Transformer {
    @Override public String id() { return "annotationSanitizer"; }
    @Override public String description() { return "remove selected runtime annotation metadata"; }
    ...
    public void transform(ObfContext ctx) {
        ...
        for (ClassNode cn : ctx.targets(id())) {
            int removed = remove(cn.visibleAnnotations, descriptors) + remove(cn.invisibleAnnotations, descriptors);
            if (removed > 0) {
                classes++;
                annotations += removed;
            }
        }
        if (annotations > 0) ctx.log().debug("annotationSanitizer removed " + annotations
