...
public final class FieldPackingTransformer implements Transformer {
    ...
    public String description() {
        return "erase instance schemas into boxed state vectors";
    }
    ...
    public void transform(ObfContext ctx) {
        ...
        for (ClassNode cn : ctx.targets(id())) {
            ...
            if (fields.size() < minFields || !hasSuperConstructor(cn)) {
                continue;
            }
            ...
            for (FieldNode fn : fields) {
                slots.put(key(cn.name, fn.name, fn.desc),
                        new Slot(cn.name, state, index++, Type.getType(fn.desc)));
            }
        ...
        if (slots.isEmpty()) {
            ctx.log().debug("fieldPacking: no eligible fields");
            return;
        }
        ...
        for (ClassNode cn : ctx.classMap().values()) {
            for (MethodNode mn : cn.methods) {
                for (AbstractInsnNode in : mn.instructions.toArray()) {
                    ...
                    if (slot == null || (field.getOpcode() != Opcodes.GETFIELD
    ...
    private static boolean eligibleClass(ClassNode cn) {
        ...
        if ((cn.access & (Opcodes.ACC_INTERFACE | Opcodes.ACC_ANNOTATION | Opcodes.ACC_ENUM
                | Opcodes.ACC_RECORD | Opcodes.ACC_MODULE)) != 0) {
            return false;
        }
        ...
        for (FieldNode fn : cn.fields) {
            if ((fn.access & Opcodes.ACC_VOLATILE) != 0) {
                return false;
            }
        }
        for (MethodNode mn : cn.methods) {
            for (AbstractInsnNode in : mn.instructions.toArray()) {
                if (in instanceof InvokeDynamicInsnNode indy
                        && indy.bsm.getOwner().equals("java/lang/runtime/ObjectMethods")) {
                    return false;
                }
            }
        }
            ...
            if (mn.name.equals("<init>")) {
                for (AbstractInsnNode in : mn.instructions.toArray()) {
                    if (in instanceof MethodInsnNode call
                            ...
                            && call.owner.equals(cn.superName)) {
                        return true;
                    }
                }
            }
        ...
        for (MethodNode mn : p.owner.methods) {
            ...
            for (AbstractInsnNode in : mn.instructions.toArray()) {
                if (!(in instanceof MethodInsnNode call)
                        || call.getOpcode() != Opcodes.INVOKESPECIAL
                        || !call.name.equals("<init>")
                        || !call.owner.equals(p.owner.superName)
                        || !call.desc.equals("()V")
                        || !receiverIsThis(call)) {
                    continue;
                }
    ...
    private static void emitUnbox(InsnList il, Type t) {
        if (t.getSort() == Type.OBJECT || t.getSort() == Type.ARRAY) {
            il.add(new TypeInsnNode(Opcodes.CHECKCAST, t.getInternalName()));
            return;
        }
