...
public final class MethodRelocationTransformer implements Transformer {
    ...
    @Override
    public String id() {
        return "methodRelocation";
    }

    @Override
    public String description() {
        return "relocate complete object method bodies across opaque carriers";
    }
    ...
    public void transform(ObfContext ctx) {
        ...
        ctx.log().debug("methodRelocation: moved " + moved + " complete bodies ("
                + instructions + " instructions) across "
                + (moved == 0 ? 0 : carriers.size()) + " carriers");
    }
    ...
    private static int eligibleScore(MethodNode mn, int minimum) {
        ...
        if ((mn.access & forbidden) != 0 || mn.name.charAt(0) == '<') {
            return -1;
        }
        if (mn.instructions == null || mn.instructions.size() < minimum || Limits.oversizeMethod(mn)) {
            return -1;
        }

        for (AbstractInsnNode in : mn.instructions.toArray()) {
            if (in instanceof InvokeDynamicInsnNode) {
                return -1;
            }
    ...
    private static List<ClassNode> carriers(ObfContext ctx, List<Candidate> candidates, int count) {
        ...
        for (Candidate c : candidates) {
            version = Math.max(version, c.owner.version & 0xFFFF);
        }
    ...
    private static MethodNode cloneBody(MethodNode source, String name, String desc) {
        ...
        if (source.tryCatchBlocks != null) {
            for (TryCatchBlockNode t : source.tryCatchBlocks) {
                copy.tryCatchBlocks.add(new TryCatchBlockNode(
                        labels.get(t.start), labels.get(t.end), labels.get(t.handler), t.type));
            }
        }
