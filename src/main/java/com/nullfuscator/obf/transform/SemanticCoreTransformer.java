...
public final class SemanticCoreTransformer implements Transformer, Opcodes {
    ...
    public void transform(ObfContext ctx) {
        ...
        for (ClassNode owner : ctx.targets(id())) {
            if (Limits.hugeClass(owner)) {
                continue;
            }
            for (MethodNode method : owner.methods) {
                if (ctx.isHotPath(owner, method)) {
                    continue;
                }
                Candidate candidate = candidate(method);
                if (candidate == null || candidate.operations < minOperations
                        || random.nextInt(100) >= percent) {
                    continue;
                }
                if (estimatedSize(method, candidate.domain) > Limits.MAX_GROW_INSNS) {
                    continue;
                }
    ...
    private static Candidate candidate(MethodNode method) {
        ...
        if ((method.access & forbidden) != 0 || (method.access & ACC_STATIC) == 0) {
            return null;
        }
        if (method.name.charAt(0) == '<' || method.instructions == null
        ...
        for (AbstractInsnNode instruction : method.instructions.toArray()) {
            ...
            if (instruction instanceof VarInsnNode variable) {
                if (opcode == ILOAD) {
                    if (!initializedLocals.contains(variable.var)) {
                        return null;
                    }
                    continue;
                }
            ...
            if (instruction instanceof IincInsnNode increment) {
                if (increment.var < 0 || !initializedLocals.contains(increment.var)) {
                    return null;
                }
                affine = true;
                operations++;
            ...
            if (opcode == DUP || opcode == DUP_X1 || opcode == DUP_X2
                    || opcode == SWAP || opcode == POP) {
                continue;
            }
            if (constantValue(instruction) != null) {
                continue;
            }
            ...
            if (operationDomain != null) {
                if (operationDomain == Domain.AFFINE) {
                    affine = true;
                } else {
                    xor = true;
                }
    ...
    private static void rewriteAffine(MethodNode method, int multiplier, int offset, int xorKey) {
        ...
        for (AbstractInsnNode instruction : original) {
            InsnList replacement = instruction instanceof JumpInsnNode jump
                    && isIntegerBranch(jump.getOpcode())
                    ? branch(jump, offset, inverse)
                    : switch (instruction.getOpcode()) {
                        case IADD -> add(offset);
                        case ISUB -> subtract(offset);
                        case IMUL -> multiply(offset, inverse);
                        case INEG -> negate(offset);
                        case ISHL, ISHR, IUSHR -> shiftThroughRaw(
                                instruction.getOpcode(), multiplier, offset, inverse);
                        case IXOR, IAND, IOR -> bitwiseThroughXor(
                                instruction.getOpcode(), multiplier, offset, inverse,
    ...
    private static InsnList bitwiseThroughXor(int opcode, int multiplier, int offset, int inverse,
                                               int xorKey, int temporaryLeft, int temporaryRight) {
        InsnList out = new InsnList();
    ...
    private static void appendMaskedBinary(InsnList out, int opcode, int key,
    ...
    private static InsnList encodedConstant(AbstractInsnNode instruction, int multiplier,
                                             int offset, int salt) {
    ...
    private static Integer constantValue(AbstractInsnNode instruction) {
        ...
        if (instruction instanceof IntInsnNode immediate
                && (immediate.getOpcode() == BIPUSH || immediate.getOpcode() == SIPUSH)) {
    ...
    private static int nextTemporaryLocal(MethodNode method) {
        ...
        for (AbstractInsnNode instruction : method.instructions) {
            if (instruction instanceof VarInsnNode variable) {
                next = Math.max(next, variable.var + 1);
            }
            if (instruction instanceof IincInsnNode increment) {
                next = Math.max(next, increment.var + 1);
            }
        }
        return next;
    }
