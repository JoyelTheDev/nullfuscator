...
public final class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            if (java.util.Arrays.asList(args).contains("--verbose")
                    || java.util.Arrays.asList(args).contains("-v")) {
    ...
    private static void run(String[] args) throws Exception {
        ...
        String first = args[0];
        if ("retrace".equals(first)) {
            retrace(args);
            return;
        }
        if ("mapping-info".equals(first)) {
            mappingInfo(args);
            return;
        }
        if ("presets".equals(first) || "preset-list".equals(first)) {
            presets();
            return;
        }
        if ("init-config".equals(first) || "init".equals(first)) {
            initConfig(args);
            return;
        }
        if ("check".equals(first) || "verify".equals(first) || "preflight".equals(first)) {
            checkJar(args);
            return;
        }
            ...
            if (arg.startsWith("-")) {
                ...
                switch (flag) {
                    ...
                    case "--lib", "-l" -> {
                        ...
                        if (libVal.contains(",")) {
                            for (String p : libVal.split(",")) {
                                if (!p.isBlank()) {
                                    additionalLibs.add(p.trim());
                                }
                            }
                        } else {
        ...
        if (noMapping && mapping != null) {
            throw new IllegalArgumentException("--mapping conflicts with --no-mapping");
        }
        if (!input.isFile()) {
        ...
        if (configArg != null && presetArg != null) {
            ...
            if (!custom.isFile()) {
                throw new IllegalArgumentException("config file does not exist: " + custom);
            }
        ...
        try {
            ...
            if (!reportOnly) {
                ...
                if (!noMapping && !ctx.mapping().isEmpty()) {
                    ctx.mapping().write(mapFile, new ObfMapping.Metadata(
                            BuildInfo.MAPPING_FORMAT, BuildInfo.VERSION, theSeed, inputHash, inputBytes));
    ...
    private static String suggestFlag(String typo) {
        ...
        for (String flag : KNOWN_FLAGS) {
            if (typo.startsWith("--") != flag.startsWith("--")) {
                continue;
            }
    ...
    private static void presets() {
        ...
        System.out.printf("%-10s %-12s %-16s %-10s %s%n",
                "Preset", "Protection", "Size Growth", "Overhead", "Recommended For");
        System.out.println("-----------------------------------------------------------------------------------------------");
    ...
    private static void initConfig(String[] args) throws Exception {
        ...
        System.out.println("[+] Configuration template created: " + out.getPath()
                + " (preset '" + preset + "')");
    }
    ...
    private static void checkJar(String[] args) throws Exception {
        ...
        for (int i = 1; i < args.length; i++) {
            String arg = args[i];
            if (arg.equals("-p") || arg.equals("--preset")) {
                preset = value(args, ++i);
            } else if (arg.equals("-c") || arg.equals("--config")) {
                config = new File(value(args, ++i));
            } else if (arg.equals("-l") || arg.equals("--lib")) {
                libs.add(value(args, ++i));
            } else if (!arg.startsWith("-") && input == null) {
                input = new File(arg);
            } else {
                throw new IllegalArgumentException("unknown arg for check: " + arg);
            }
        }
    ...
    private static void validatePaths(File input, File output, File mapping, File report,
        ...
        if (config != null) {
            protectedFiles.add(config);
        }
        for (String lib : cfg.libs()) {
            protectedFiles.add(new File(lib));
        }
        ...
        for (int i = 0; i < destinations.length; i++) {
            ...
            for (int j = 0; j < i; j++) {
                if (destinations[j] != null && sameFile(destination, destinations[j])) {
                    throw new IllegalArgumentException(
                            "output, mapping and report paths must be distinct: " + destination);
                }
            }
            for (int j = 0; j < protectedFiles.size(); j++) {
                ...
                if (sameFile(destination, protectedFiles.get(j))) {
                    throw new IllegalArgumentException(
                            "destination would overwrite an input or configuration: " + destination);
                }
            }
        }
    }
