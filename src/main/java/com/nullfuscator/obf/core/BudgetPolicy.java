...
public final class BudgetPolicy {
    ...
    public static void verifyStructure(ObfContext ctx) {
        ...
        if (report == null || report.baseline() == null || report.result() == null) {
            return;
        }
        ...
        check("class count", before.classes(), after.classes(),
                budget.getInt("maxClassGrowthPercent", -1),
                budget.getInt("classGrowthAllowance", 0));
        check("method count", before.methods(), after.methods(),
                budget.getInt("maxMethodGrowthPercent", -1),
    ...
    public static void verifyJar(ObfContext ctx, long outputBytes) {
        if (ctx.report() != null) {
            ctx.report().outputJarBytes(outputBytes);
        }
    ...
    private static void verifyCoverage(ObfContext ctx, RunReport report) {
        ...
        for (String key : report.baseline().methods().keySet()) {
            ...
            if (!selectors.matches(owner) && !selectors.matchesMethod(owner, name, descriptor)) {
                continue;
            }
            total++;
            if (report.changedMethods().contains(key)) {
                changed++;
            }
        }

        if (total == 0) {
            throw new IllegalArgumentException("coverage.required matched no input methods");
        }

        int minimum = section.getInt("minPercent", 1);
        int actual = (int) ((changed * 100L) / total);
        if (actual < minimum) {
            throw new IllegalStateException("required coverage is " + actual
                    + "% (" + changed + "/" + total + "), below " + minimum + "%");
        }
    }

    private static void check(String label, long before, long after,
                              int maximumPercent, long allowance) {
        if (maximumPercent < 0 || after <= before) {
            return;
        }
        long growth = after - before;
        long allowed = allowance + before * maximumPercent / 100;
        if (growth > allowed) {
            throw new IllegalStateException(label + " grew by " + growth
