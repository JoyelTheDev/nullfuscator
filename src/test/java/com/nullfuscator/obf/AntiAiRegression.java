        ...
        for (int seed = 0; seed < 30; seed++) {
            for (int level = 1; level <= 3; level++) {
                ObfContext ctx = new ObfContext(
                        ObfConfig.parse("antiAI { level:" + level
                                + ", decoyMethods:8, constantEncodingPercent:100 }"),
                        seed,
                        new ObfLog(false),
                ...
                long[] edges = {
                        ...
                        Long.MIN_VALUE, Long.MAX_VALUE, 0xffffffffL, 0x80000000L,
                        127, 128, -129, 32767
                };
                ...
                for (int i = 0; i < 120; i++) {
                    ...
                    Object constant;
                    if (wide) {
                        constant = Long.valueOf(value);
                    } else {
                        constant = Integer.valueOf((int) value);
                    }
                    ...
                    if (!wide && (int) value >= -1 && (int) value <= 5) {
                        ...
                        mn.instructions.add(new LdcInsnNode(constant));
                    }
                ...
                for (int i = 0; i < 120; i++) {
                    GrowthRegression.check(
                            expected.get(i).equals(loaded.getMethod("m" + i).invoke(null)),
                ...
                GrowthRegression.check(
                        !expected.get(first).equals(broken.getMethod("m" + first).invoke(null)),
                        "helper removal did not affect observable result");
            }
        }

        System.out.println("PASS antiAI: " + checks + " differential values, 30 seeds, "
                + "levels 1-3, Java 8 classes / Java 17 interfaces, live-helper mutation");
    }
}